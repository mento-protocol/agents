...
# section of the single-file script, less remove_link, which link.sh carries
...
# Writes: OUT_COUNT. err, which it calls, raises ERRORS, and record_output
# fills the output table this run writes the manifest back from.
#
# main calls this section as 'if ! cmd_unlink', so errexit is off in its whole
...
# refusal is 'return 1', which cmd_unlink raises again, so the run ends where
