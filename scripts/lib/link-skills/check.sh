...
# Writes: nothing of its own. output_err, which it calls, raises ERRORS, and
# the loading it does fills the source, candidate and manifest tables.
#
# main calls this section as 'if ! check_cmd', so errexit is off in its whole
# subtree: every report helper is called bare and ends no run of its own.
# check_cmd itself can end the run through output_die, so it is never called
# in a substitution.
...
_check_runtime_link() {
	...
	home_dir=$1
	if [ ! -d "$home_dir" ]; then
		output_info "  skipped $home_dir/skills: $home_dir does not exist"
		return 0
	fi
	...
	if [ -L "$link" ]; then
		...
		if paths_same "$cur" "$ASSEMBLY_DIR"; then
			output_info "  runtime ok: $link -> $ASSEMBLY_DIR"
			return 0
		fi
		output_err "runtime problem: $link -> $cur, expected $ASSEMBLY_DIR"
		return 0
	fi
	if [ -e "$link" ]; then
		output_err "runtime problem: $link is not a symlink to $ASSEMBLY_DIR"
		return 0
	fi
	output_err "runtime problem: $link is missing; run '$PROG link'"
}
...
check_cmd() {
	...
	if [ ! -f "$SOURCES_FILE" ]; then
		output_die "no sources file at $SOURCES_FILE; run '$PROG link' inside a clone to create one"
	fi
	...
	if ! manifest_load; then
		output_err "could not read the manifest $MANIFEST"
		return 1
	fi
	sources_load
	if [ "$SRC_COUNT" -eq 0 ]; then
		output_die "no source is listed in $SOURCES_FILE; add one skills directory per line"
	fi
	candidates_collect
	candidates_report_empty_sources

	_check_report_sources

	_check_report_candidates

	_check_report_manifest

	_check_runtime_link "$HOME/.claude"
	_check_runtime_link "$HOME/.codex"
...
_check_report_sources() {
	...
	while [ "$i" -lt "$SRC_COUNT" ]; do
		...
		if paths_same "$src" "$ASSEMBLY_DIR"; then
			output_info "source $src: is the assembly directory itself"
			continue
		fi
		...
		if [ "${SRC_OK[$((i - 1))]-0}" != "1" ]; then
			output_info "source $src: cannot be read"
			continue
		fi
...
_check_report_git() {
	...
	src=$1
	output_info "source $src: ok"
	if ! root=$(git_root "$src"); then
		output_info "  git: not a clone"
		return 0
	fi
	...
	if [ "$behind" = "unknown" ] && ! git_default_branch "$root" >/dev/null 2>&1; then
		output_info "  git: default branch unknown; run: git remote set-head origin --auto"
	fi
	output_info "  fetch: $fetch_note"
}
...
_check_report_candidates() {
	...
	output_info "assembly $ASSEMBLY_DIR:"
	i=0
	while [ "$i" -lt "$CAND_COUNT" ]; do
		...
		if [ -e "$entry" ]; then
			output_err "link collision: $name exists in the assembly and this script did not create it"
			continue
		fi
		output_err "link missing: $name ($target)"
	done
}
...
_check_report_symlink() {
	...
	if manifest_entry_is_recorded_link "$name" "$entry"; then
		...
		if candidates_target_source_unavailable "$cur" "$oldspell"; then
			output_info "  link kept: $name; its source cannot be read now"
			return 0
		fi
	fi
	if [ ! -e "$entry" ]; then
		output_err "link dangling: $name -> $cur"
		return 0
	fi
	...
	if [ "$recorded" = "1" ]; then
		output_err "link stale: $name -> $cur, expected $target; run '$PROG link'"
		return 0
	fi
	output_err "link collision: $name is a foreign symlink to $cur"
	return 0
}
...
_check_report_manifest() {
	...
	while [ "$i" -lt "$MAN_COUNT" ]; do
		...
		i=$((i + 1))
		if candidates_index_of "$name" >/dev/null; then
			continue
		fi
		...
		if candidates_dup_has "$name"; then
			output_info "  link kept: $name; more than one source provides it"
			continue
		fi
		entry="$ASSEMBLY_DIR/$name"
		...
		if candidates_unreadable_has "$name"; then
			if [ -L "$entry" ]; then
				output_info "  link kept: $name; its skill directory cannot be read now"
			fi
			continue
		fi
...
_check_report_orphan() {
	...
	if [ -L "$entry" ] && [ ! -e "$entry" ]; then
		output_err "link dangling: $name (recorded target $target)"
		return 0
	fi
	if [ -L "$entry" ]; then
		cur=$(paths_link_target_abs "$entry")
		...
		if paths_same "$cur" "$target"; then
			output_err "link orphan: $name; '$PROG link' will prune it"
		fi
	fi
}
