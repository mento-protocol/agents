...
# in every one of them. A refusal here is output_die, which ends the run, and
# one line and exit 0 when the run is a session hook.
...
config_resolve_sources() {
	...
	if [ "$SOURCES_SET" -eq 1 ]; then
		...
		"/") output_die "--sources must name a file, not /" ;;
		esac
	...
	SOURCES_FILE=$(paths_abs "$(paths_expand_home "$SOURCES_FILE")")
	case "$(paths_normalize_lexical "$SOURCES_FILE")" in
	"" | "/") output_die "the sources file must not be / or empty" ;;
	esac
	# paths_canonical prints nothing, and a failed assignment would leave the
	# spelling behind empty, so the path is held here for the refusal to name.
	spelled=$SOURCES_FILE
	if ! SOURCES_FILE=$(paths_canonical "$spelled"); then
		output_die "the sources file path $spelled runs through a name that is not a directory"
	fi
...
config_resolve_assembly() {
	...
	if [ "$ASSEMBLY_SET" -eq 1 ]; then
		...
		"/") output_die "--assembly must name a directory below /, not / itself" ;;
		esac
	...
	ASSEMBLY_DIR=$(paths_abs "$(paths_expand_home "$ASSEMBLY_DIR")")
	case "$(paths_normalize_lexical "$ASSEMBLY_DIR")" in
	"" | "/") output_die "the assembly directory must not be / or empty: it would put every skill link in the filesystem root" ;;
	esac
	spelled=$ASSEMBLY_DIR
	if ! ASSEMBLY_DIR=$(paths_canonical "$spelled"); then
		output_die "the assembly directory path $spelled runs through a name that is not a directory"
	fi
	...
	if runtime_assembly_holds_link; then
		output_die "the assembly directory must not contain a runtime skills path: $ASSEMBLY_DIR"
	fi
...
config_default_paths() {
	...
	DEFAULT_SOURCES_FILE=$(paths_canonical "$DEFAULT_SOURCES_FILE" 2>/dev/null) ||
		DEFAULT_SOURCES_FILE="$HOME/.agents/skill-sources"
	DEFAULT_ASSEMBLY_DIR="$HOME/.agents/skills"
	DEFAULT_ASSEMBLY_DIR=$(paths_canonical "$DEFAULT_ASSEMBLY_DIR" 2>/dev/null) ||
		DEFAULT_ASSEMBLY_DIR="$HOME/.agents/skills"
...
config_guard_sources() {
	...
	# _runtime_ensure_sources_file. A path that names one of this script's own
	# control paths inside the assembly would have a run read its bookkeeping
	# as a list of sources, or write a sources file over it. The comparison is
	# on the canonical paths, so an alias is refused as well as the plain
	# spelling, and it runs before anything is read, written or created.
	if sources_is_control_path; then
		output_die "the sources file must not be an assembly control file: $SOURCES_FILE"
	fi
	# Judged once, here, for every command: the readers below all reach this
	# path, and the bootstrap in _runtime_ensure_sources_file writes to it.
